...
skip_signature
	lda consol			;Disable cart if option is pressed by entering the simple menu		
	and #4
...
;	While ramtop ($6a) and ramsiz ($2e4) are change to $c0 during
...
not_finished_user_break
	...
	beq not_specific_genre			;If the search genre is not specific, continue with comparing the title
...
;substring_end_found
	...
	position = x1				;ZP variables

	mva #menu_entry.title position		;Compare entry with input buffer
	ldy #menu_entry.title_length		;end_position = title position+<title_length>
	adc (search_pointer),y
	sta compare_end_position		;Self-modifying code to save cycles
...
compare_end_position = *+1
	cpy #$ff
	bcs entry_not_matching
	lda (search_pointer),y
	cmp #'a'
	bcc no_lower_case
	cmp #'z'+1
	bcs no_lower_case
...
no_lower_case
	cmp input_buffer,x
	bne no_char_match_found			;Restart compare for substring at next position
	iny
	inx
compare_input_length = *+1
	cpx #$ff
...
next_entry
	inw search_entry_index			;Increament entries index
