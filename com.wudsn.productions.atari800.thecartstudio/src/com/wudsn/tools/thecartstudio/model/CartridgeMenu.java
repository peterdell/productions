...
public final class CartridgeMenu {
			...
			if (root.getCartridgeMenuType() == CartridgeMenuType.SIMPLE) {
				...
				if (content != null) {
					...
					setWord(content, Offsets.MENU_GENRES_COUNT, root
							.getUnmodifiableGenresList().size() + 1); // +1
					// for
					// "All"
	...
	public static void setMenuEntriesContent(Workbook workbook, byte[] result,
		...
		allGenre.setName(WorkbookGenre.ALL);
		List<WorkbookGenre> genresList = new ArrayList<WorkbookGenre>(
				root.getUnmodifiableGenresList());
		genresList.add(0, allGenre);
		int genreCount = genresList.size();
		int genreNameOffset = genreCount * bytesPerGenre;
		for (int i = 0; i < genreCount; i++) {
			WorkbookGenre genre = genresList.get(i);
			int offset = genresOffset + i * bytesPerGenre;
			setWord(result, offset, genreNameOffset);
		...
		System.arraycopy(genreNamesBytes, 0, result, genreNameOffset,
				genreNamesBytes.length);
		...
		for (int i = 0; i < workbookMenuEntries.size(); i++) {
			...
			// Grouping and filter properties are write in the third step.
			offset = entryOffset + Offsets.MENU_ENTRY_GENRE_NUMBER;
			int genreNumber = 0; // allGenre
			...
			setByte(result, offset, genreNumber);

			offset = entryOffset + Offsets.MENU_ENTRY_FAVORITE_INDICATOR;
			setByte(result, offset, workbookMenuEntry.getWorkbookEntry()
					.getFavoriteIndicator() ? 1 : 0);
