...
public final class AtrFile {
	 ...
	 */
	private AtrFile(byte[] atrData) {
		if (atrData == null) {
			throw new IllegalArgumentException(
					"Parameter 'atrData' must not be null.");
		}
		if (!isHeader(atrData)) {
			throw new IllegalArgumentException(
					"Parameter 'atrData' is not a valid ATR file");
		}
		this.atrData = atrData;

		sectorSize = (atrData[4] & MASK_FF) + ((atrData[5] & MASK_FF) << 8);

		int paragraphs = (atrData[2] & MASK_FF);
		paragraphs += (atrData[3] & MASK_FF) << 8;
		paragraphs += (atrData[6] & MASK_FF) << 16;
		paragraphs += (atrData[7] & MASK_FF) << 24;
		int dataSize = paragraphs * PARAGRAPH_SIZE;
		if (atrData.length >= BOOT_SECTORS_SIZE_SD) {
			sectorCount = (atrData.length - BOOT_SECTORS_SIZE_SD) / sectorSize
					+ BOOT_SECTORS;
		} else {
	 ...
	 */
	public static boolean isHeader(byte[] atrData) {
		if (atrData == null) {
			throw new IllegalArgumentException(
					"Parameter 'data' must not be null.");
		}
		if (atrData.length >= 16 && atrData[0] == (byte) 0x96
				&& atrData[1] == (byte) 0x02) {
			return true;
		}
		return false;
	 ...
	 * @throws AtrException
	 *             If the atrData contains an invalid ATR.
	 */
	public static AtrFile createInstance(byte[] atrData) throws AtrException {
		if (atrData == null) {
			throw new IllegalArgumentException(
					"Parameter 'atrData' must not be null.");
		}
		...
		AtrFile result = new AtrFile(atrData);
		return result;
	}
	...
	public byte[] getSector(int sectorNumber) throws AtrException {
		...
		System.arraycopy(atrData, offset, sector, 0, sectorSize);
		return sector;
	}
